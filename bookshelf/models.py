...
#             raise ValueError("The profile photo must be set")
#         email = self.normalize_email(email)
#         user = self.model(date_of_birth=date_of_birth, profile_photo=profile_photo, email=email)
#         user.set_password(password)
#         user.save(using=self._db)
#         return user
       
#     def create_superuser(self, email, date_of_birth, profile_photo, password=None):
#         user = self.create_user(email=email, date_of_birth=date_of_birth, profile_photo=profile_photo, password=password)
#         user.is_staff = True
...
class CustomUser(AbstractUser):
    ...
    # PROFILE_PIC_FIELD = "profile_photo"
